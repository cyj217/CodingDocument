...
# 添加变量：节点访问顺序
u = model.addVars(range(num_cities), vtype=grb.GRB.INTEGER, lb=0, ub=num_cities-1, name="u")
...
# 添加约束：MTZ禁止子回路
model.addConstrs(u[i] >= 1 for i in range(1, num_cities))
model.addConstrs(u[i] <= num_cities - 1 for i in range(1, num_cities))
model.addConstrs(u[i] - u[j] + (num_cities - 1) * x[i, j] <= num_cities - 2 for i in range(1, num_cities) for j in range(1, num_cities) if i != j)
